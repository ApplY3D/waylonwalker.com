...
cover_image: https://images.waylonwalker.com/reasons-to-kedro.png
...
``` bash
conda create -n my-new-project -y python=3.8 kedro new kedro install kedro run
```
...
If kedro does not have a `DataSet` for the format that you need to read or write you can easily create your own custom `DataSet`  all you need to do is inherit from `kedro.io.AbstractDataSet` and create methods for `__init__`,
...
``` python
from os.path import isfile from typing import Any, Union, Dict
...
There is a growing list of plugins available from pypi that is only a `pip install` away.  Most of them are on [GitHub](https://github.com/topics/kedro-plugin) and tagged as a [kedro-plugin](https://github.com/topics/kedro-plugin) topic.
