---
canonical_url: https://waylonwalker.com/how-i-deploy-2022/
cover_image: https://images.waylonwalker.com/how-i-deploy-2022.png
...
Twitter is a great networking tool, but it's rare to see anything more than a few hours old.
...
I could not do any of this if I was focused on Building rather than writing.
...
I use [copier](https://copier.readthedocs.io/en/stable/) for single file templates.


## Copier give me a new page

How I Present from the terminal with lookatme lookatme-slides
...
!TIP Once it starts getting uncomfortable to find posts, its nice to have good shortcuts to get around.
...
Often times I am working away on some sort of project, and I just need to save a snippet for a later post.
...
I have a vim hotkey `gic` to commit my current file, and `gpp` to push it.
...
In March of 2021 I made the big switch from a javascript based framework to my own ssg.

## I thought it would be easy....

There are a bunch of open source libraries that do all the things I need an ssg to do.

## Moving to python

One of the biggest selling points to moving back to python was that I use it every day and know the ecosystem much better.
...
``` python
pymdown-extensions python-frontmatter
```

## frontmatter

All the metadata is defined in yaml frontmatter and read in with the [python-frontmatter](https://github.com/eyeseast/python-frontmatter) library.

``` yaml
---
templateKey: blog-post tags: ['webdev', 'meta' ] title: How I deploy my blog in 2022 date: 2021-10-29 status: draft
...
[Pluggy](https://pluggy.readthedocs.io/en/stable/) is what I use to implement my lifecycle.
...
Pluggy allows the framework to crate a `hook_spec` and plugin authors to implement hooks with the `hook_impl`.
...
``` python
pm = pluggy.PluginManager("markata") pm.add_hookspecs(hookspec.MarkataSpecs)
...
[Diskcache](https://github.com/grantjenks/python-diskcache/) allows you to setup a persistent cache layer.
...
If the content is not yet set or has expired, you will get `None` back and need to create the value.

``` python
html_from_cache = cache.get(key) if html_from_cache is None:
...
[anyconfig](https://github.com/ssato/python-anyconfig) is a great tool to pull your config from generic config files.
...
Anyconfig needs a `path`, `parser`, and `keys`.  The key is your tools prefix
...
Each key in the config files used with `anyconfig` must be prefixed with the tool's name.

```
# markata.toml
[markata] default_cache_expire = 1209600

[markata.auto_description.description] len=160
```
...
```
mkdir pages echo '# My First Post' > first-post.md echo '# Hello World' > hello-world.md
```
...
Don't worry about having the perfect post, just make something that is useful to you, and others who will find it.
