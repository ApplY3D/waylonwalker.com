---
canonical_url: https://waylonwalker.com/tmux-nav-2021/
cover_image: https://images.waylonwalker.com/tmux-nav-2021.png
description: 'In 2021 I changed the way I navigate between tmux sessions big time.  Now
  I can In 2021 I changed the way I navigate between tmux sessions big time.  Now
  I can '
published: true
tags:
- cli
...
Since making this post, I have made ~20 other posts in short form that all have a YouTube video to go along with them you can find them all on my [tmux-playlist](https://www.youtube.com/playlist?list=PLTRNG6WIHETB4reAxbWza3CZeP9KL6Bkr).
...
Before the course, I was proficient in navigating within each of my tmux sessions but rarely started more than one session.  A few months later, I have adopted a lot of what I learned from Chris and made it my own.
...
Another article can dive into my `ta` command. This one is more about the methodology, workflow, and keybindings to get me there.  It's available in my [script](https://github.com/WaylonWalker/devtainer/blob/main/bin/.local/bin/ta).
...
I have set up to make it easy to open my non-work projects _(in my ~/git directory)_ and my work projects _(in my ~/work directory)_.  I bound `prefix+c-g` and
`prefix+c-w` to open a new session in their respective directories.  I like
mapping common prefix commands with control to keep my pinky mashed on that control key.

``` bash
# ~/.tmux.conf

bind C-w new-window -n "work-session-picker" "ta ~/work" bind C-g new-window -n "git-session-picker" "ta ~/git"
```
...
Now that I have `ta` rocking with a good create or attach setup, I am rarely toggling through a list of running sessions, but I am doing it with
...
Next and Previous sessions.  This is super handy when working with under 3 sessions to be able to cycle through sessions holding `shift+alt` and pressing
`n` or `p`.
...
While `M-n` and `M-p` work well with a small, focused number of sessions, I often end up with too many sessions open, and it's not efficient to remember a double
...
Please let me know your thoughts. [@waylonwalker](https://twitter.com/_WaylonWalker), this one took me a bit longer to put together with all of the animated gif's, but I think it helps visually show how I navigate tmux every day.

## Please give it a share if you liked it

If you liked it, give it a share and tag me on [twitter](https://twitter.com/_WaylonWalker).  I don't often ask but this article took a bit more to put together than my normal post.
