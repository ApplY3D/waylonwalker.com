---
canonical_url: https://waylonwalker.com/what-is-kedro-1/
cover_image: https://images.waylonwalker.com/what-is-kedro-1.png
...
``` python
import pandas as pd bikes_df = pd.DataFrame({...<bikes_data>...}) catalog.datasets.bikes.save(bikes_df)
```
...
``` python
import pandas as pd import numpy as np from kedro.pipeline import Pipeline from kedro.pipeline import node
...
``` bash
cd my-kedro-project kedro docker build kedro docker run
```
