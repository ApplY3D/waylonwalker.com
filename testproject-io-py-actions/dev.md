...
cover_image: https://images.waylonwalker.com/testproject-io-py-actions.png
...
import time import pytest from src.TestProject.sdk.drivers import web driver
...
``` bash
trap 'kill $(jobs -p)' EXIT attempt_counter=0 max_attempts=100 mkdir -p build/reports/agent docker-compose -f docker-compose.yml logs -f | tee build/reports/agent/log.txt& until curl -s http://localhost:8585/api/status | jq '.registered' | grep true; do
